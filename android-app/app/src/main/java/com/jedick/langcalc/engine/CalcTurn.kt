 ...
 *
 * [errorType] is null unless [status] is ERROR from a failed expression evaluation; it lets the UI
 * pick a localized message (see LangCalcStrings in ChatScreen.kt). A null [errorType] with ERROR
 * status means the error came from the model (inference failure) instead.
 */
data class CalcTurn(
  ...
  val errorType: CalcExpressionEvaluator.EvalErrorType? = null,
)
    ...
    .fold(
      ...
      onFailure = { error ->
        copy(
          ...
          resultText = "",
          errorType = (error as? CalcExpressionEvaluator.EvalException)?.type
              ?: CalcExpressionEvaluator.EvalErrorType.SYNTAX,
        )
      },
    )
